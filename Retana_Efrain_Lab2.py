...
def draw_rectangle(I,r,c,h,w): # (r,c) coordinates for rectangle, h,w for height 
    # and width of the rectangle
    img,img_gl = read_image(I)
    rect_points = np.array([ [r,c],[r+w,c],[r+h,c+w],[r,c+w] ])
    ax1.plot(rect_points[:,0],rect_points[:,1],linewidth = 2.0,color ='red')
    ax2.plot(rect_points[:,0],rect_points[:,1],linewidth = 2.0,color ='red')
    
def brightest_pixel(I,r,c,h,w):
...
if __name__ == "__main__":
    ...
    for file in img_files:
        ...
        if file[-4:] == '.png': # File contains an image
            ...
            draw_rectangle(img_dir+file,0,0,100,100)
            plt.show()
